...
def process_movies(movies_file, links_file, output_file):
    ...
    # Load data and ensure that imdbId is treated as a string
    ...
    for index, row in merged_df.iterrows():
        ...
        if r1 is None:
            ...
            if pd.notna(tmdb_title):
                ...
                if result is None:  # Check if the function returned None
                    abstract, director = "Abstract not available", "Director not available"
                    # abstract = "Abstract not available"
                else:
                    abstract, director = result  # Safe decomposition
        ...
        # Save every X rows to avoid losing progress
        if index % 5 == 0:  # Save every 10 rows
            merged_df.to_csv(output_file, index=False)
            print(f"Backup saved: {output_file} (up to row {index})")
