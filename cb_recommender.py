...
class ContentBasedRecommender:
    ...
    def _preprocess_text(self) -> None:
        ...
        self.tokenized_abstracts = [
            [word for word in simple_preprocess(text) if word not in STOPWORDS] for text in self.df[self.abstract_col]
        ]
    ...
    def _compute_genres_similarity(self, idx : int ) -> np.ndarray :
        ...
        genre_sims = np.array([
            self._jaccard_similarity(target_genres, curr_genres_list) for curr_genres_list in self.genres_lists
            ]) 
        return genre_sims

    def compute_similarity_scores(self, idx: int, alpha=0.90) -> np.ndarray:
        # Calcola la similarità basata sull'abstract (cosine similarity)
        # Confronta l'embedding del film dato con gli embeddings di tutti gli altri film
        ...
        # Combina le due similarità: abstract e generi usando il parametro alpha
        final_sim = alpha * abstract_sim + (1 - alpha) * genre_sims
